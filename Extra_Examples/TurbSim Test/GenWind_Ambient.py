...
URefInit = 7 # Starting URef (m/s)
URefMod = 0 # How much to increase URef by per iteration (m/s)
WindDirInit = 8 # Starting HFlowAng of wind (degrees)
WindDirMod = 0 # How much to increase HFlowAng by per iteration (degrees)

turbsimEXE = "TurbSim_x64.exe" # TurbSim exe file location
turbsimINP = "TurbSim_Ambient.inp" # TurbSim input file location

numGrid = 30 # must match turbsimIMP file, both x and y
gridLength = 300 # meters, must match turbsimIMP file, both height and width (163 minimum for NREL 5MW baseline (found through trial/error))
rotorRadius = 126/2 # meters, diameter of 126 for 5 MW turbine
refHt = 90 # meters, must match turbsimIMP file
# gridLength >= rotorRadius + refHt or genData will read nonexistent values
...
def writeGenData():
    ...
    with open("Generator_Ambient/genData" + str(i) + ".csv", 'w') as genData:
        genData.write("Point A,Point B,Point C,Point D,Point E\n")
        j = 13 # starting point of the first grid, line 14 in the .u file
        while j<len(uLines):
            dataAppend = ""
            # Equation for y: numGrid*(rotorRadius/gridLength)*sin(θ) + numGrid - numGrid(refHt/gridLength)
            # Equation for x: numGrid*(rotorRadius/gridLength)*cos(θ) + 1/2*numGrid
            # y is the first brackets, x is the second brackets
            dataAppend += uLines[j+ round(numGrid*((rotorRadius/gridLength)*-1.000000 + (1 - refHt/gridLength)))] [3+8*round(numGrid*(rotorRadius/gridLength)* 0.000000 + 0.5*numGrid) : 3+8*round(numGrid*(rotorRadius/gridLength)* 0.000000 + 0.5*numGrid)+6] + "," # Point A, Top-Center   point of star by 90-72*0 degrees
            dataAppend += uLines[j+ round(numGrid*((rotorRadius/gridLength)*-0.309017 + (1 - refHt/gridLength)))] [3+8*round(numGrid*(rotorRadius/gridLength)* 0.951057 + 0.5*numGrid) : 3+8*round(numGrid*(rotorRadius/gridLength)* 0.951057 + 0.5*numGrid)+6] + "," # Point B, Top-Right     point of star by 90-72*1 degrees
            dataAppend += uLines[j+ round(numGrid*((rotorRadius/gridLength)* 0.809017 + (1 - refHt/gridLength)))] [3+8*round(numGrid*(rotorRadius/gridLength)* 0.587785 + 0.5*numGrid) : 3+8*round(numGrid*(rotorRadius/gridLength)* 0.587785 + 0.5*numGrid)+6] + "," # Point C, Bottom-Right  point of star by 90-72*2 degrees
            dataAppend += uLines[j+ round(numGrid*((rotorRadius/gridLength)* 0.809017 + (1 - refHt/gridLength)))] [3+8*round(numGrid*(rotorRadius/gridLength)*-0.587785 + 0.5*numGrid) : 3+8*round(numGrid*(rotorRadius/gridLength)*-0.587785 + 0.5*numGrid)+6] + "," # Point D, Bottom-Left point of star by 90-72*3 degrees
            dataAppend += uLines[j+ round(numGrid*((rotorRadius/gridLength)*-0.309017 + (1 - refHt/gridLength)))] [3+8*round(numGrid*(rotorRadius/gridLength)*-0.951057 + 0.5*numGrid) : 3+8*round(numGrid*(rotorRadius/gridLength)*-0.951057 + 0.5*numGrid)+6] + "," # Point E, Top-Left    point of star by 90-72*4 degrees
            j += numGrid + 2 # go to next grid (there's a gap of 2 lines between each grid)
            genData.write(dataAppend + "\n")

def writeFullData():
    with open("Generator_Ambient/fullData" + str(i) + ".csv", 'w') as genData:
...
for i in range (1,numIterations+1):
    ...
    turbsimINP = "Generator_Ambient/TurbSim" + str(i) + ".inp"
    ...
    # Write TurbSim log into a file
    with open("Generator_Ambient/TurbSim" + str(i) + "Log.txt", 'w') as file:
    ...
    # Write machine learning input data to a file
    if genData:
        writeGenData()

    # Rewrite data from the .u file as a csv file
    if fullData:
        writeFullData()
